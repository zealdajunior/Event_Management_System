...
class TicketGeneratorService
{
    ...
    public function generateQRCode(Booking $booking): string
    {
        ...
        if (!$attendance) {
            throw new \Exception('Attendance record not found for this booking');
        }
    ...
    public function getTicketData(Booking $booking): array
    {
        ...
        return [
            ...
            'booking_reference' => 'BKG-' . str_pad($booking->id, 8, '0', STR_PAD_LEFT),
            'user_name' => $booking->user->name,
            'user_email' => $booking->user->email,
            'event_title' => $booking->event->title,
            'event_description' => $booking->event->description,
            'event_date' => $booking->event->date->format('F d, Y'),
            'event_time' => $booking->event->date->format('g:i A'),
            'event_venue' => $booking->event->venue->name ?? 'TBA',
            'event_address' => $booking->event->venue->address ?? 'TBA',
            'ticket_type' => $booking->ticket->type,
            'ticket_price' => number_format($booking->ticket->price, 2),
            'booking_date' => $booking->booking_date->format('F d, Y g:i A'),
