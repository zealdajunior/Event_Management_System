                ...
                <div class="bg-white rounded-lg shadow p-6 mb-6">
                    ...
                    <div class="space-y-3">
                        <div class="flex justify-between items-center">
                            ...
                            <span class="text-sm font-medium text-gray-900" id="stats-total-events">{{ $stats['total_events'] }}</span>
                        </div>
                        <div class="flex justify-between items-center">
                            <span class="text-sm text-gray-600">Featured</span>
                            <span class="text-sm font-medium text-indigo-600" id="stats-featured-events">{{ $stats['featured_events'] }}</span>
                        </div>
                        <div class="flex justify-between items-center">
                            <span class="text-sm text-gray-600">Free Events</span>
                            <span class="text-sm font-medium text-green-600" id="stats-free-events">{{ $stats['free_events'] }}</span>
                        </div>
                        <div class="flex justify-between items-center">
                            <span class="text-sm text-gray-600">Revenue</span>
                            <span class="text-sm font-medium text-gray-900" id="stats-revenue">${{ number_format($stats['total_revenue'], 2) }}</span>
                        </div>
                    </div>
                </div>
            ...
            <form id="calendar-filters-form" class="space-y-4">
                ...
                <div>
                    ...
                    <select id="filter-venue" name="venue_id" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 rounded-md">
                        ...
                        @foreach($filters['venues'] as $venue)
                            <option value="{{ $venue->id }}">{{ $venue->name }} - {{ $venue->city }}, {{ $venue->state }}</option>
                        @endforeach
                    </select>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function searchEvents(query) {
        fetch(`{{ route('calendar.search') }}?query=${encodeURIComponent(query)}&type=events`)
    ...
    function loadEventsForDate(dateStr) {
        fetch(`{{ route('calendar.events-for-date') }}?date=${dateStr}`)
            .then(response => response.json())
            .then(data => {
    ...
    function updateStats() {
        ...
        fetch(`{{ route('calendar.stats') }}?start=${start}&end=${end}`)
            .then(response => response.json())
            .then(data => {
