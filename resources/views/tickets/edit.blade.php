        ...
        <div class="bg-gradient-to-r from-purple-900/80 via-pink-900/80 to-green-900/80 backdrop-blur-xl rounded-3xl p-6 mb-8 shadow-2xl border border-white/10">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-white/70 mt-2">Update the ticket details</p>
                </div>
            </div>
        </div>
    </x-slot>
                                ...
                                <div>
                                    ...
                                    <select id="event_id" class="block mt-1 w-full bg-white/10 border-white/20 text-white focus:border-purple-400 focus:ring-purple-400 rounded-xl" name="event_id" required>
                                        ...
                                        @foreach($events as $event)
                                            <option value="{{ $event->id }}" {{ old('event_id', $ticket->event_id) == $event->id ? 'selected' : '' }} class="bg-gray-800">{{ $event->name }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="bg-white/5 backdrop-blur-sm rounded-2xl p-6 border border-white/10">
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                    <div>
                                        ...
                                        <x-text-input id="type" class="block mt-1 w-full bg-white/10 border-white/20 text-white placeholder-white/50 focus:border-purple-400 focus:ring-purple-400 rounded-xl" type="text" name="type" :value="old('type', $ticket->type)" required autofocus autocomplete="type" placeholder="e.g., General, VIP, Early Bird" />
                                    ...
                                    <div>
                                        ...
                                        <x-text-input id="price" class="block mt-1 w-full bg-white/10 border-white/20 text-white placeholder-white/50 focus:border-purple-400 focus:ring-purple-400 rounded-xl" type="number" step="0.01" name="price" :value="old('price', $ticket->price)" required min="0" placeholder="0.00 for free tickets" />
                                    ...
                                    <div>
                                        ...
                                        <x-text-input id="quantity" class="block mt-1 w-full bg-white/10 border-white/20 text-white placeholder-white/50 focus:border-purple-400 focus:ring-purple-400 rounded-xl" type="number" name="quantity" :value="old('quantity', $ticket->quantity)" required min="1" placeholder="Number of tickets available" />
