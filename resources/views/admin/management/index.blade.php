        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                ...
                <div class="p-6">
                    @forelse($superAdmins as $superAdmin)
                        <div class="flex items-center justify-between border-b border-gray-200 py-3 last:border-0">
                            <div class="flex items-center space-x-3">
                                <div class="flex-shrink-0 h-10 w-10 rounded-full bg-gradient-to-r from-purple-500 to-blue-500 flex items-center justify-center text-white font-bold">
                                    {{ strtoupper(substr($superAdmin->name, 0, 1)) }}
                                </div>
                                <div>
                                    <p class="text-sm font-medium text-gray-900">{{ $superAdmin->name }}</p>
                    ...
                    @empty
                        <p class="text-gray-500 text-center py-4">No super admins found</p>
                    @endforelse
                </div>
            </div>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                ...
                <div class="p-6">
                    <form method="POST" action="{{ route('admin.admins.create') }}" class="space-y-4">
                        @csrf
            ...
            <!-- Current Admins -->
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div class="p-6 bg-gradient-to-r from-blue-500 to-indigo-500 text-white">
                    <h3 class="text-lg font-semibold">Current Admins ({{ $admins->count() }})</h3>
                </div>
                <div class="p-6">
                                            ...
                                            <div class="flex space-x-2">
                                                @if(!$admin->is_super_admin)
                                                    ...
                                                    <form method="POST" action="{{ route('admin.users.toggle-super', $admin) }}" class="inline">
                                                        @csrf
                                                        <button type="submit" class="text-purple-600 hover:text-purple-900" title="Grant Super Admin">
                                                    ...
                                                    <form method="POST" action="{{ route('admin.users.demote', $admin) }}" class="inline">
                                                        @csrf
                                                        <button type="submit" class="text-yellow-600 hover:text-yellow-900" title="Demote to User" onclick="return confirm('Are you sure you want to demote this admin?')">
                                                ...
                                                @else
                                                    @if($admin->id !== auth()->id())
                                                        ...
                                                        <form method="POST" action="{{ route('admin.users.toggle-super', $admin) }}" class="inline">
                                                            @csrf
                                                            <button type="submit" class="text-orange-600 hover:text-orange-900" title="Revoke Super Admin" onclick="return confirm('Are you sure you want to revoke super admin privileges?')">
                                                                <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                ...
                                                @if($admin->id !== auth()->id())
                                                    <!-- Delete Admin -->
                                                    <form method="POST" action="{{ route('admin.users.delete', $admin) }}" class="inline">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="text-red-600 hover:text-red-900" title="Delete Admin" onclick="return confirm('Are you sure you want to delete this admin? This action cannot be undone.')">
                                                            <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-gradient-to-r from-gray-600 to-gray-700 text-white">
                    <h3 class="text-lg font-semibold">Regular Users</h3>
                </div>
                <div class="p-6">
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse($users as $user)
                                    <tr>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="flex items-center">
                                                <div class="flex-shrink-0 h-10 w-10 rounded-full bg-gray-500 flex items-center justify-center text-white font-bold">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <div class="flex space-x-2">
                                                ...
                                                <form method="POST" action="{{ route('admin.users.promote', $user) }}" class="inline">
                                                    @csrf
                                                    <button type="submit" class="text-green-600 hover:text-green-900" title="Promote to Admin">
                                                        <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
